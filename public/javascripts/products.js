...
const updatePagination = (pages, currentPage, currentFunction, catId = '', sort = '') => {
    ...
    pages.forEach(page => {
        ...
        if (sort) {
            link.href = `javascript:${currentFunction}(${page}, '${catId}', '${sort}');`;
        } else {
            link.href = `javascript:${currentFunction}(${page}, '${catId}');`;
        }
        link.innerText = page;
...
///new arrivals
// const newArrivals = async (page = 1) => {
//     const sort = -1;
//     //console.log('Sort:', sort);
//     const response = await fetch(`/sort_product_arrivals`, {
//         method: 'POST',
//         headers: {
//             'Content-Type': 'application/json'
//         },
...
// ///popularity
// const popularity = async (page = 1) => {
//     const sort = -1;
//     //console.log('Sort:', sort);
//     const response = await fetch(`/sort_product_popularity`, {
//         method: 'POST',
//         headers: {
//             'Content-Type': 'application/json'
//         },
...
const newArrivals = async (page = 1) => {
    ...
    if (data.productData.length > 0) {
        filteredDataDisplay(data.productData);
        updatePagination(data.pages, data.currentPage, 'newArrivals', catId, sort);
    } else {
        proContainer.innerHTML = `<h2 class="m-5">No product available</h2>`;
        clearPagination();
    }
...
const popularity = async (page = 1) => {
    ...
    if (data.productData.length > 0) {
        filteredDataDisplay(data.productData);
        updatePagination(data.pages, data.currentPage, 'popularity', catId, sort);
    } else {
        proContainer.innerHTML = `<h2 class="m-5">No product available</h2>`;
        clearPagination();
    }
